...
if [[ -f "fixes.yaml" ]]; then
    ...
    # If the fixes.yaml file contains some errors, then we need to stage the changes again
    if grep -q error "fixes.yaml"; then
        result=1
    # If it only contains warnings, we can ignore them
    else
        result=0
    fi
    rm fixes.yaml
